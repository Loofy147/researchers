...
def generate_report():
    ...
    transactions = qs.get("research_transactions", [])
    regime = qs.get("regime_analysis", {})
...
"""
    ...
    if transactions:
        ...
        doc += "Identification of 'flows' where high-utility frameworks can improve neighboring models based on coordinate proximity:\n\n"
        for tx in transactions:
            doc += f"### {tx['transaction_id']}: {tx['source']} → {tx['target']}\n"
            doc += f"**Improvement Potential: {tx['improvement_potential']:.3f}**\n\n"
            doc += f"- **Transacted Assets**: {', '.join(tx['transacted_assets'])}\n"
            doc += f"- **Coordinate Distance**: {tx['distance']:.3f}\n\n"

    if regime:
        doc += "--- \n\n## V. Regime Stability & Phase-Shift Analysis (SLA-2.1)\n\n"
        doc += f"**Current Status**: {regime['regime_status']}\n\n"
        doc += f"- **Entropy (Max Deviation)**: {regime['entropy_max_dev']:.3f}\n"
        doc += f"- **Blend Factor (α)**: {regime['alpha']:.3f}\n"
        doc += f"- **Kill-Switch Trigger**: {regime['kill_switch_trigger']}\n\n"
        if regime['ste_active']:
            doc += "**WARNING**: State-Transition Event detected. Vector B (Mechanical Necessity) is exceeded by 2-Sigma.\n\n"
